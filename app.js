...
const API_BASE_URL = 'https://capstone-backend-mnwh.onrender.com';
...
async function saveTeam(team) {
    try {
        ...
        if (result.analysis) {
            ...
            return 'similarity'; // Return special value for similarity check
        }
        ...
        showError('ideas-error', result.error || 'Registration failed.');
        return false;

    } catch (error) {
        console.error('Error saving team:', error);
        showError('ideas-error', 'Error saving team: ' + error.message);
        return false;
    }
}
...
function showIdeaSimilarityWarning(similarArray, teamData, onSubmitAnyway) {
    ...
    (teamData.project_ideas || []).forEach((idea, idx) => {
        msg += `\n${idx+1}) "${idea}" ${similarArray[idx] ? "⚠ Similar" : "✅ Unique"}`;
    });
...
async function handleProjectIdeasForm(event) {
    ...
    const result = await saveTeam(currentTeam);
    ...
    if (result === true) {
        ...
        // This is an actual error (not similarity check)
        // The saveTeam function already showed the error message
        // Only show generic error if no specific error was shown
        const errorElement = document.getElementById('ideas-error');
    ...
    // If result === 'similarity', do nothing - similarity warning is already shown
...
window.submitTeamWithForceRegister = submitTeamWithForceRegister;
